...
==================================
Cytoplasmic IHC analysis (CK, Vimentin, S100, etc.).

Measures DAB intensity within the cytoplasm (cell body minus nucleus).
"""
...
def analyze_cytoplasmic_ihc(image, params):
    """
    Cytoplasmic IHC analysis pipeline.
    ...
    # Approximate cytoplasm: dilate nucleus, subtract nucleus
    cyto_expansion = params.get("ring_width", 6)
    ...
    for rp in measure.regionprops(labels):
        ...
        cyto_pixels = dab[cyto_mask]

        if cyto_pixels.size == 0:
            md = 0.0
        else:
            md = float(np.mean(cyto_pixels))
    ...
    result = {
        ...
        "allred_score": "N/A (cytoplasmic marker)",
        "cell_data": cell_data,
        "labels": labels,
    }
    ...
    logger.info("Cytoplasmic IHC analysis complete: %d cells, H-score=%.1f", total, hs)
    return result
